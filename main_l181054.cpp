...
#include<string>
#include<fstream>
bankAccount** readfile(int &noOfacc,string filepath); 
void printAccounts(bankAccount **arr,int size);
bool makeTransaction(bankAccount**arr1,int size,int ID1, float amount,int a);
void deleteAccounts(bankAccount** arr2,int size);
...

{
	...
	//	declaring and initializebn can be done at same time using following syntax
	...
	bankAccount **dyarray=new bankAccount*[2];
	float transactions1[2]={-5000,5000};
	dyarray[0]=new bankAccount(50,2000,transactions1,2);
	dyarray[1]=new bankAccount(60,6000,400000);
	dyarray[0]->withdraw(1000);
	dyarray[0]->print();
	dyarray[1]->print();
	...
	//	*/
	int noOfacc;
	bankAccount **newarray=readfile(noOfacc,"data.txt");
	...
	//	*/
	printAccounts(newarray,noOfacc);
	//	/********************************TASK 8*************************************
	...
	int input=-1;
	//here as required to perform the withdrawal
	input=1;
	if(input=1)
	{
		makeTransaction(newarray,noOfacc,5,6000,1);
	}
	else
	{
		makeTransaction(newarray,noOfacc,5,6000,0);
	}
	//	/********************************TASK 9*************************************
...

{
	...
	infile>>noOfacc;
	bankAccount **array=new bankAccount*[noOfacc];
	for(int i=0; i<noOfacc;i++)
	{
		...
		int nooftrans;
		infile>>nooftrans;
		float *transaction=new float[nooftrans];
		for(int j=0; j<nooftrans;j++)
		{
			infile>>transaction[j];
		}
		array[i]=new bankAccount(id,balance,transaction,nooftrans,balancelimit);
	}
	return array;
}
void printAccounts(bankAccount **arr,int size)
...
bool makeTransaction(bankAccount**arr1,int size,int ID1, float amount,int a)
{
	...
	for(int j=0; j<size;j++)
	{
		if(arr1[j]->getID()==ID1);
		{
			if(a==1)
			{
				arr1[j]->deposit(amount);
				
			}
			else
			{
				arr1[j]->withdraw(amount);
			}
			return true;
		}
